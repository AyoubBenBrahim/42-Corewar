...
char	*get_color(char *str, int begin, int end)
{
	...
	color_name = pf_strsub(str, begin, end - begin + 1);
	i = 0;
	while (i < COLORS)
	{
		if (!pf_strcmp(color_name, g_colors[i].color_name))
		{
			pf_strdel(&color_name);
...
void	set_color(t_vars *var)
{
	...
	if (var->str[j] == '{')
	{
		...
		while (var->str[j] != '}')
		{
			...
			{
				var->counter += pf_putchar(var->str[var->i]);
				return ;
			}
			j++;
		}
		if ((color = get_color(var->str, var->i + 1, j - 1)))
			write(1, color, pf_strlen(color));
		var->i = j;
	}
	else
		var->counter += pf_putchar(var->str[var->i]);
