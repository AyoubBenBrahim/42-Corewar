...
void	is_s(t_args *args, int len)
{
	...
	if (args->precision == 0)
		...
		args->result = pf_strsub(args->result, 0, args->precision);
...
void	pf_poubx(t_args *args, int len)
{
	int		zeros;
	char	*tmp;
	char	*result;
...
void	pf_precision(t_args *args)
{
	...
	var.elt = args->result;
	var.len = pf_strlen(var.elt);
	if (args->spec == 's' && args->precision >= 0)
		is_s(args, var.len);
	else if (pf_contains("poubxX", args->spec))
		pf_poubx(args, var.len);
	else if (pf_contains("id", args->spec))
	{
		var.sign = (var.elt[0] == '-');
		...
		if (args->precision > var.len - var.sign)
		{
			...
			args->result = var.sign ? \
							pf_strjoin("-", args->result) : args->result;
			if (var.sign)
				pf_strdel(&var.tmp);
		}
	}
