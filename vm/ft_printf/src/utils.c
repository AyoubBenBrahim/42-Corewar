...
int		is_inf(t_args *arg)
{
	...
	flag = !pf_strcmp(arg->result, "inf") || !pf_strcmp(arg->result, "-inf") \
		|| !pf_strcmp(arg->result, "+inf") || !pf_strcmp(arg->result, " inf");
	return (flag);
}
...
int		manage_sign(t_args *arg, int sign)
{
	if (!sign)
	{
		if ((arg->flag & MASK_PLUS) && arg->spec != 'u')
		{
			...
			{
				arg->result = pf_strjoin_free(pf_strdup("+"), arg->result);
				return (1);
			}
			return (0);
		}
