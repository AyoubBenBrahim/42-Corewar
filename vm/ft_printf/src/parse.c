...
{
	while (pf_contains("-+#0 ", var->str[var->j]))
	{
		if (var->str[var->j] == '-')
		{
...
{
	while (pf_contains("hlLzj", var->str[var->j]))
	{
...
void	get_width(t_args *args, t_vars *var)
{
	...
	while (pf_isdigit(var->str[var->j]) || var->str[var->j] == '*')
	{
		if (pf_isdigit(var->str[var->j]))
		{
			w = pf_atoll(&var->str[var->j]);
...
{
	while (pf_contains("-+#* .jzhlL", var->str[var->j]) \
			|| pf_isdigit(var->str[var->j]))
		var->j++;
	if (var->str[var->j])
	{
