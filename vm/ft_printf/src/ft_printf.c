...
**	or from format string using pf_atoll function
*/
...
static void	va_arg_precision(t_args *args, t_vars *var)
{
	...
	{
		args->precision = pf_atoll(&var->str[var->j]);
		if (args->precision < 0)
		{
			if (args->spec == 'f')
				args->precision = 6;
...
void		get_precision(t_args *args, t_vars *var)
{
	if (var->str[var->j] == '.')
	{
		...
		while (pf_isdigit(var->str[var->j]))
			var->j++;
		if ((args->flag & MASK_ZERO) && args->precision >= 0 \
				&& pf_contains(DIOUXXPB, args->spec))
			args->flag ^= MASK_ZERO;
	}
	else if (args->spec == 'f')
		args->precision = 6;
...
{
	if (pf_contains("id", args->spec))
		pf_print_d(args, var);
	else if (pf_contains("poubxX", args->spec))
		pf_print_poubx(args, var);
	else if (args->spec == 's')
		pf_print_s(args, var);
	else if (args->spec == 'f')
		pf_print_f(args, var);
...
static void	pf_printf_helper(t_args *args, t_vars *var)
{
	while (var->str[++var->i])
	{
		if (var->str[var->i] == '%')
...
/*
**	Our evil pf_printf trying to imitate printf monster 😈
*/
